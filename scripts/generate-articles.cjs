  ...
  return `
    <script>
        ...
        async function submitArticle(event) {
            ...
            try {
                // Submit to NocoDB via proxy (handles authentication)
                const response = await fetch('https://sorcery-api-proxy.pedro-4e6.workers.dev/api/v2/tables/article_submissions/records', {
                    method: 'POST',
                    headers: {
